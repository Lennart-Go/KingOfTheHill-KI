...
bool isInsufficientMatingMaterial(t_board *board) {
    ...
    if ((board->white & (board->knight | board->king)) > 0 &&
        (board->white & (board->queen | board->rook | board->bishop | board->pawn)) == 0) {
        return true;
    }
    if ((board->black & (board->knight | board->king)) > 0 &&
        (board->black & (board->queen | board->rook | board->bishop | board->pawn)) == 0) {
        return true;
    }
    //king and one bishop
    if ((board->king & board->white && board->white & board->bishop) > 0 &&
        (countFigure(board->bishop & board->white)) < 2 &&
        (board->white & (board->queen | board->rook | board->bishop | board->pawn)) == 0) {
        return true;
    }

    if ((board->king & board->black && board->black & board->bishop) > 0 &&
        countFigure(board->bishop & board->black) < 2 &&
        (board->black & (board->queen | board->rook | board->bishop | board->pawn)) == 0) {
        return true;
    }
    return false;
...
winner_t checkEnd(t_board *board, bool moved_color) {
    ...
    } else if (isCheckmate(board, !moved_color)) {
        return moved_color ? BLACK : WHITE;
    } else if (isStalemate(board, !moved_color)) {
        return DRAW;
//    } else if (isInsufficientMatingMaterial(board)) {
//        // Insufficient mating material senseless in king-of-the-hill
//        return DRAW;
    } else {
        return NOTOVER;
    }
